	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nOi Mamaco Bem vindo *${mdata.subject}*`
				...
				teks = `N�o Faz falta mb @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnoton: '❬ X ❭ *Vixx fracote*',
				levelnol: '*Pqp kskst level* 0 ',
				error: {
					stick: '[❗] Oh mamaco ta vendo q n funfa ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					ownerG: '[❗] Este comando só S� os admiro random ❌',
					ownerB: '[❗] Este comando só pode ser usado pela Yunno  ❌',
